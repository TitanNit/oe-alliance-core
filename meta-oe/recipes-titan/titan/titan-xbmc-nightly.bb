...
PNBLACKLIST[xbmc] += "/usr/include/c++/ctime:71:11: error: '::localtime' has not been declared"

PV = "auto+nigthly+gitr${SRCPV}"
PR = "r14"
...
#	wget https://github.com/xbmc/FFmpeg/archive/2.4.6-Helix.tar.gz
	...
	cd ${S}/tools/depends/native/libsquish-native
	make
#	make DESTDIR=${STAGING_DIR_TARGET} install
#    export PKG_CONFIG_PATH="${S}/tools/depends/native/libsquish-native/squish-install/lib/pkgconfig:$PKG_CONFIG_PATH"
echo cp ${S}/tools/depends/native/libsquish-native/squish-install/* ${STAGING_DIR_TARGET}/usr

cp -a ${S}/tools/depends/native/libsquish-native/squish-install/* ${STAGING_DIR_TARGET}/usr
